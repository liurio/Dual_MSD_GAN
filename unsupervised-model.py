...
class DualNet(object):
    ...
    def load_samples(self):
        self.train_data_A,self.train_data_B ,self.val_data_A,self.val_data_B= data_files()
    ...
    def load_random_samples(self):
        ...
        sample_files =np.random.choice(self.val_data_A,self.batch_size)
        sample_A_imgs = [load_data(f, image_size =self.image_size, is_test = True,flip=self.flip) for f in sample_files]
        
        sample_files = np.random.choice(self.val_data_B,self.batch_size)
    ...
    def train(self, args):
        ...
        for epoch_idx in xrange(args.epoch):
            ...
            data_A,data_B = self.train_data_A,self.train_data_B
            ...
            for batch_idx in xrange(0, epoch_size):
                ...
                print("Epoch: [%2d] [%4d/%4d],loading time:[%4.4f]"%(epoch_idx, batch_idx, epoch_size,time.time()-start_time))
                step = step + 1
    ...
    def run_optim(self,batch_A_imgs, batch_B_imgs,  counter, start_time):
        ...
        print('Ad,Bd trianing time:%4.4f'%(time.time()-start_time))
        _, Ag, Bg, Aloss, Bloss = self.sess.run(
            [self.g_optim, self.Ag_loss, self.Bg_loss, self.A_loss, self.B_loss], 
            feed_dict={ self.real_A: batch_A_imgs, self.real_B: batch_B_imgs})

        print('Ag,Bg trianing time:%4.4f' % (time.time() - start_time))
        _, Ag, Bg, Aloss, Bloss = self.sess.run(
            [self.g_optim, self.Ag_loss, self.Bg_loss, self.A_loss, self.B_loss], 
            feed_dict={ self.real_A: batch_A_imgs, self.real_B: batch_B_imgs})
    ...
    def discriminator(self, image,  y=None, prefix='A_d_', reuse=False):
        ...
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            ...
            h3 = conv2d(h2, 1, d_h=1, d_w=1, name =prefix+'h3')
            return h3
    ...
    def fcn(self, imgs, prefix=None, reuse = False):
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            ...
            # e8 is (1 x 1 x self.fcn_filter_dim*8)
                ...
                if prefix == 'B_g_':
                    self.d7, self.d7_w, self.d7_b = deconv2d(tf.nn.relu(d6), [self.batch_size, s, s, self.A_channels],
                                                             name=prefix + 'd7', with_w=True)
                elif prefix == 'A_g_':
                    self.d7, self.d7_w, self.d7_b = deconv2d(tf.nn.relu(d6), [self.batch_size, s, s, self.B_channels],
                                                             name=prefix + 'd7', with_w=True)
                # d8 is (256 x 256 x output_c_dim)
